...
{
    public class CPostShippingCalcPlugin : IShippingRateCalculationProvider
    {
        #region Fields
        ...
        private readonly CPostShippingSettings _dpdShippingSettings;

        #endregion

        #region Ctor
        public CPostShippingCalcPlugin(
            ...
            CPostShippingSettings dpdShippingSettings)
        {
        ...
        {
            return 0;
            //var shippingCPostService = _serviceProvider.GetRequiredService<ICPostShippingService>();
            //var shippingCPostSettings = _serviceProvider.GetRequiredService<CPostShippingSettings>();

            //var shippingCPostRecord = await shippingCPostService.FindRecord(shippingMethodId,
            //    storeId, warehouseId, countryId, stateProvinceId, zip, weight);
            //if (shippingCPostRecord == null)
            //{
            //    if (shippingCPostSettings.LimitMethodsToCreated)
            //        return null;
            ...
            //double shippingTotal = shippingCPostRecord.AdditionalFixedCost;
            ////charge amount per weight unit
            //if (shippingCPostRecord.RatePerWeightUnit > 0)
            //{
            //    var weightRate = weight - shippingCPostRecord.LowerWeightLimit;
            //    if (weightRate < 0)
            //        weightRate = 0;
            //    shippingTotal += shippingCPostRecord.RatePerWeightUnit * weightRate;
            //}
            ////percentage rate of subtotal
            //if (shippingCPostRecord.PercentageRateOfSubtotal > 0)
            //{
            //    shippingTotal += Math.Round((double)((((float)subTotal) * ((float)shippingCPostRecord.PercentageRateOfSubtotal)) / 100f), 2);
            //}
        ...
        public Task<GetShippingOptionResponse> GetShippingOptions(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            response.ShippingOptions.Add(new ShippingOption() {
                Name = _translationService.GetResource("Shipping.CPost.PluginName"),
                Description = _translationService.GetResource("Shipping.CPost.PluginDescription"),
                Rate = 0,
                ShippingRateProviderSystemName = "Shipping.CPost"
            });
        ...
        public string ConfigurationUrl => CPostShippingDefaults.ConfigurationUrl;

        public string SystemName => CPostShippingDefaults.ProviderSystemName;

        public string FriendlyName => _translationService.GetResource(CPostShippingDefaults.FriendlyName);
        ...
        public async Task<IList<string>> ValidateShippingForm(string shippingOption, IDictionary<string, string> data)
        {
            ...
            if (string.IsNullOrEmpty(shippingOptionId))
                return new List<string>() { _translationService.GetResource("Shipping.CPost.SelectBeforeProceed") };

            if (shippingMethodName != _translationService.GetResource("Shipping.CPost.PluginName"))
            ...
            CPostPointModel serializedObject = Newtonsoft.Json.JsonConvert.DeserializeObject<CPostPointModel>(shippingOptionObjectJson);

            var stringBuilder = new StringBuilder();
            string serializedAttribute;
            await using (var tw = new StringWriter(stringBuilder))
            {
                var xmlS = new XmlSerializer(typeof(CPostPointModel));
        ...
        {
            return await Task.FromResult("Plugins.CPost.Points");
        }
